      ...
      >
        <div
          className={`px-3.5 py-2 text-[15px] leading-relaxed ${
            decryptFailed
              ? "border-danger/25 bg-danger/8 text-danger inline-flex items-center gap-2 rounded-2xl border"
              : fromSelf
        ...
        >
          {decryptFailed ? (
            <>
              ...
              <span>Could not decrypt</span>
            </>
          ) : (
            (text ?? "…")
          )}
        </div>
        <span className="label-mono tnum px-1">{formatTime(m.created_at)}</span>
      </div>
    </div>
  );
};
